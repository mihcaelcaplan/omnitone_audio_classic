...
// #define NRF_SIDECHANNEL_ON //uncomment for sidechannel block
...
static void bt_app_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    case ESP_BT_GAP_ACL_DISCONN_CMPL_STAT_EVT:
        ...
        status_flags = (status_flags & (~EXT_MCU_BT_FLAG));
        break;
    /* others */
    default: {
        ESP_LOGI(BT_AV_TAG, "event: %d", event);
...
void app_main(void)
{
    ...
    err = init_SPI();
    if(err != ESP_OK){
        ESP_LOGE("OMNI", "spi not Initialized");
    }
    bt_spi_task_start_up();
    ESP_LOGI("OMNI","SPI Task started :)");
